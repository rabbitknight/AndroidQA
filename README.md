...
#### [Q] 抽象类与接口的应用场景
#### [Q] 抽象类是否可以没有方法和属性？
#### [Q] 简单工厂模式/工厂方法模式和抽象工厂模式
1. A1:  
   + 简单/静态工厂模式：同一工厂，不同方法创建不同对象。Factory::createDog/createCat
   + 工厂方法模式：同一创建方法，不同工厂创建不同对象。DogFactory:create/CatFactory:create
   + 抽象工厂模式：同一类工厂，不同方法创建不同对象，不同厂家添加不同属性。AbstractFactory.createDog/createCat;FemaleFactory.createDog/createCat;MaleFactory.createDog/createCat;
